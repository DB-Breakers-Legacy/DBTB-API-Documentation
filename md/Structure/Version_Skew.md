...
| 15 | Heartbeat server plaintext | lobby2 notes: client's truncated public IP:port (`b'33.22:53431'`-style) | **Captures** | 26 B = `01` + echoed ms counter + `a0 01 00 00` + the **server-observed** public address. (The heartbeat transcript annotation — `heartbeat.txt` in [dev-amto/private-logs](https://github.com/dev-amto/private-logs), collaborators only — was corrected in M5.) |
